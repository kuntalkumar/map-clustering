...
const MiniMap = () => {
  ...
  useEffect(() => {
    ...
    // Initialize the minimap and sync with the main map
    if (minimapRef.current === null) {
      minimapRef.current = L.map("minimap", {
        center: mapRef.current.getCenter(),
        zoom: mapRef.current.getZoom() - 2, // Adjust zoom level
        dragging: true,
        zoomControl: true,
      ...
      // Sync zoom levels between the main map and the minimap
      mapRef.current.on("zoom", () => {
        minimapRef.current.setZoom(mapRef.current.getZoom() - 2); // Sync zoom level with a slight offset
      });

      minimapRef.current.on("zoom", () => {
        mapRef.current.setZoom(minimapRef.current.getZoom() + 2); // Sync zoom level back
      });

      // Sync the minimap with the main map movement
      mapRef.current.on("move", () => {
        minimapRef.current.setView(mapRef.current.getCenter(), minimapRef.current.getZoom());
      });

      // Clicking on the minimap will adjust the main map position
  ...
  return (
    <div>
      <h2 style={{ display: "flex", justifyContent: "center" }}>Map with Minimap</h2>
